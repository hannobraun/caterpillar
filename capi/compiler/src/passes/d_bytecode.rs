...
pub fn generate_bytecode(fragments: Fragments) -> (Bytecode, SourceMap) {
    ...
    for call in compiler.calls_to_user_defined_functions {
        let Some(function) = compiler.functions_by_name.get(&call.name) else {
            unreachable!(
    ...
    let bytecode = Bytecode {
        ...
        function_arguments: compiler.functions_by_address,
    };
...
struct Compiler<'r> {
    ...
    functions_by_address: BTreeMap<InstructionAddress, Vec<String>>,
